...
async fn main() {
    ...
    let engine = MemoryEngine::new(storage, embedder, 512).expect("Failed to create engine");
    ...
    let (_m1_saved, _edge, _m2_saved) = engine.store_with_edge(m1, m2, RelationType::Causal).await.unwrap();

    // 3. Search for "Aria"
    println!("Searching for 'Aria'...");
    let results = engine.search_by_text("Aria", 5).await.unwrap();
